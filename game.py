...
siglas = ("H", "He", "Li", "Be", "B", "C", "N", "O", "F", "Ne", "Na", "Mg", "Ai", "Si", "P", "S", "Cl", "Ar", "K", "Ca", "Sc", "Ti", "V", "Cr", "Mn", "Fe", "Co", "Ni", "Cu", "Zn", "Ga", "Ge", "As", "Se", "Br", "Kr", "Rb", "Sr", "Y", "Zr", "Nb", "Mo", "Tc", "Ru", "Rh", "Pd", "Ag", "Cd", "In", "Sn", "Sb", "Te", "I", "Xe", "Cs", "Ba", "Hf", "Ta",
          "W", "Re", "Os", "Ir", "Pt", "Au", "Hg", "Ti", "Pb", "Bi", "Po", "At", "Rn", "Fr", "Ra", "Rf", "Db", "Sg", "Bh", "Hs", "Mt", "Ds", "Rg", "Cn", "Nh", "Fl", "Mc", "Lv", "Ts", "Og", "La", "Ce", "Pr", "Nd", "Pm", "Sm", "Eu", "Gd", "Tb", "Dy", "Ho", "Er", "Tm", "Yb", "Lu", "Ac", "Th", "Pa", "U", "Np", "Pu", "Am", "Cm", "Bk", "Cf", "Es", "Fm", "Md")
nomes = ("Hidrogênio", "Hélio", "Lítio", "Berílio", "Boro", "Carbono", "Nitrogênio", "Oxigênio", "Flúor", "Neônio", "Sódio", "Magnésio", "Alumínio", "Silício", "Fósforo", "Enxofre", "Cloro", "Argônio", "Potássio", "Cálcio", "Escândio", "Titânio", "Vanádio", "Cromo", "Manganês", "Ferro", "Cobalto", "Níquel", "Cobre", "Zinco", "Gálio", "Germânio", "Arsênio", "Selênio", "Bromo", "Criptônio", "Rubídio", "Estrôncio", "Ítrio", "Zircônio", "Nióbio", "Molibdênio", "Tecnécio", "Rutênio", "Ródio", "Paládio", "Prata", "Cádmio", "Índio", "Estanho", "Antimônio", "Telúrio", "Iodo", "Xenônio", "Césio", "Bário", "Háfnio", "Tântalo", "Tungstênio", "Rênio",
...
dificultyrect = pygame.Rect(xwindow/2-128, ywindow/2+400, 150, 900)
...
dificulty = pygame.sprite.Sprite(startgroup)
# create a green solid button dificulty
dificulty.image = pygame.Surface((256, 129))
dificulty.image.fill((152, 195, 121))
dificulty.rect = pygame.Rect(xwindow/2-128, ywindow/2+400, 150, 900)

actual_dificulty = "easy"
# Cores
...
def show_win_message():
    ...
    win_text = fonttitle.render(
        "Parabens, você acertou todas", True, ("#000000"))
...
def show_lose_message(questoes_acertadas):
    display.fill(COR_FUNDO_DERROTA)  # Cor vermelha para derrota
    lose_text = fonttitle.render(
        "boa tentativa, você acertou: " + questoes_acertadas, True, ("#000000"))
    ...
    if started == True:
        ...
        if started == False and isintutorial == False and not isincredits:
            title = fonttitle.render(
                "Jogo Da Tabela Periódica", True, ("#D8DEE9"))
            text_title = title.get_rect(center=(xwindow/2, ywindow/2-300))
            display.blit(title, text_title)
            if actual_dificulty == 'easy':
                easy = fontdescript.render("Fácil", True, ("#D8DEE9"))
                text_easy = easy.get_rect(center=(xwindow/2, ywindow/2+465))
                display.blit(easy, text_easy)
                # set dificulty color to green
                dificulty.image.fill("#a2bd8c")
            if actual_dificulty == 'medium':
                medium = fontdescript.render("Médio", True, ("#D8DEE9"))
                text_medium = medium.get_rect(
                    center=(xwindow/2, ywindow/2+465))
                display.blit(medium, text_medium)
                # set dificulty color to orange
                dificulty.image.fill("#cf8770")
            if actual_dificulty == 'hard':
                hard = fontdescript.render("Difícil", True, ("#D8DEE9"))
                text_hard = hard.get_rect(center=(xwindow/2, ywindow/2+465))
                display.blit(hard, text_hard)
                # set dificulty color to red
                dificulty.image.fill("#BF616A")

        if isintutorial:
            tutorialtitle = fonttitle.render("Como jogar", True, ("#88C0D0"))
            text_tutorialtitle = tutorialtitle.get_rect(
                center=(xwindow/2, ywindow/2-300))
            display.blit(tutorialtitle, text_tutorialtitle)

            tutorialdescrip = fontdescript.render(
                """Quando começar a jogar voce verá que tem um timer de 20 segundos, quando o timer chegar a 0 você perde.""", True, ("#D8DEE9"))
            text_tutorialdescrip = tutorialdescrip.get_rect(
                center=(xwindow/2, ywindow/2-200+200))
            display.blit(tutorialdescrip, text_tutorialdescrip)
            tutorialdescrip2 = fontdescript.render(
                """Aparecerá aleatoriamente siglas da tabela periódica, seu objetivo é dizer quais são os elementos""", True, ("#D8DEE9"))
            text_tutorialdescrip2 = tutorialdescrip2.get_rect(
                center=(xwindow/2, ywindow/2-175+200))
            display.blit(tutorialdescrip2, text_tutorialdescrip2)
            tutorialdescrip3 = fontdescript.render(
                """referentes as siglas, toda vez que voce acertar o timer é reiniciado, quando você acertar 5 elementos""", True, ("#D8DEE9"))
            text_tutorialdescrip3 = tutorialdescrip3.get_rect(
                center=(xwindow/2, ywindow/2-150+200))
            display.blit(tutorialdescrip3, text_tutorialdescrip3)
            tutorialdescrip4 = fontdescript.render(
                """você ganha.""", True, ("#D8DEE9"))
            text_tutorialdescrip4 = tutorialdescrip4.get_rect(
                center=(xwindow/2, ywindow/2-125+200))
            display.blit(tutorialdescrip4, text_tutorialdescrip4)

            exittutorial = fontdescript.render(
                """Clique fora da janela para fechar o Tutorial""", True, ("#4C566A"))
            text_exittutorial = exittutorial.get_rect(
                center=(xwindow/2, ywindow/2-125+500))
            display.blit(exittutorial, text_exittutorial)

        elif isincredits:
            creditstitle = fonttitle.render("Créditos", True, ("#88C0D0"))
            text_creditstitle = creditstitle.get_rect(
                center=(xwindow/2, ywindow/2-300))
            display.blit(creditstitle, text_creditstitle)

            tutorialdescrip = fontnames.render("Renan", True, ("#A3BE8C"))
            text_tutorialdescrip = tutorialdescrip.get_rect(
                center=(xwindow/2, ywindow/2-200))
            display.blit(tutorialdescrip, text_tutorialdescrip)
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if mouseposx > xwindow/2-(256/2)+270 and mouseposx < xwindow/2-(256/2)+270+256 and mouseposy > ywindow-300 and mouseposy < ywindow-300+129 and isintutorial == False and started == False:
                    isincredits = True
                if mouseposx > xwindow/2-128 and mouseposx < xwindow/2-128+256 and mouseposy > ywindow/2+400 and mouseposy < ywindow/2+400+129 and isintutorial == False and started == False:
                    if actual_dificulty == 'easy':
                        actual_dificulty = 'medium'
                    elif actual_dificulty == 'medium':
                        actual_dificulty = 'hard'
                    elif actual_dificulty == 'hard':
                        actual_dificulty = 'easy'
